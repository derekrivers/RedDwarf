...
| 105 | **[BLOCKED]** Docker sandboxing for developer phase - run the Developer phase in a per-session Docker sandbox for execution isolation | blocked | OpenClaw container image does not include an inner `docker` CLI; `sandbox.mode: "non-main"` fails with "docker not found in PATH". All agents currently use `sandbox: { mode: "off" }`. Outer container boundary is the isolation layer. Unblocked only if the deployment moves to a Docker-in-Docker capable image. See TROUBLESHOOTING.md. |
