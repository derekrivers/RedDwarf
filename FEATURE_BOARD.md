...
| 90       | Atomic run claiming for each pipeline phase - replace read-then-write overlap checks with database-backed ownership claims and stale-run takeover in one path | M15 | pending | Control Plane, Concurrency and Conflict Handling, Evidence Plane      |
| 91       | Transactional manifest, approval, phase, evidence, and run-event transitions - commit each logical state change as one database transaction | M15 | pending | Control Plane, Evidence Plane, Approval and Risk Model                |
