...
### Phase 3 -- Defensive Depth ✅ COMPLETE

| # | Feature | Priority | Gaps Addressed | Status |
|---|---------|----------|----------------|--------|
| R-09 | **Retry jitter on all backoff paths** -- Added random jitter (0.5x–1.5x multiplier) to OpenClaw dispatch retry in both `HttpOpenClawDispatchAdapter` and `AcpxOpenClawDispatchAdapter`, and to both `computeBackoffMs` functions in the polling daemon. Delays are capped at 30s (dispatch) and `MAX_BACKOFF_MS` (polling). Prevents synchronized retry storms after transient failures. | P3 | G-09 | **Done** |
| R-10 | **Disk space pre-check before evidence/workspace writes** -- `assertDiskSpaceAvailable()` in `packages/control-plane/src/workspace.ts` uses `fs.statfs()` to check free disk space before `materializeManagedWorkspace` and `archiveEvidenceArtifact`. Throws `DiskSpaceError` when below configurable threshold (`REDDWARF_MIN_DISK_FREE_MB`, default 500). Silently skips if `statfs()` fails on non-existent paths. | P3 | G-08 | **Done** |
| R-11 | **Postgres query retry for transient connection errors** -- `withTransientRetry()` in `PostgresPlanningRepository` wraps `healthcheck()`, `saveEvidenceRecord()`, and `saveGitHubIssuePollingCursor()` with a single retry on transient Postgres errors (codes 57P01/57P02/57P03/08000/08003/08006) and syscall errors (ECONNRESET/ECONNREFUSED/EPIPE/ETIMEDOUT). Does not retry inside transactions. | P3 | G-11 | **Done** |
| R-12 | **OpenClaw session cancellation on crash recovery** -- `sweepStaleRuns()` accepts an optional `cancelOpenClawSession` callback. After marking a run stale, it fetches the manifest, builds the session key via `buildOpenClawIssueSessionKeyFromManifest()`, and attempts `DELETE /hooks/sessions/:key` to release orphaned agent sessions. Best-effort with try-catch — failures are logged but never block the sweep. Wired into both startup and periodic sweeps in `start-stack.mjs`. Result includes `cancelledSessionKeys` array. 2 new tests. | P3 | G-14 | **Done** |

### Phase 4 -- Chaos Testing Infrastructure ✅ COMPLETE

| # | Feature | Priority | Gaps Addressed | Status |
|---|---------|----------|----------------|--------|
| R-13 | **Toxiproxy integration for integration tests** -- `docker-compose.chaos.yml` overlay inserts Toxiproxy (ghcr.io/shopify/toxiproxy:2.11.0) between the host process and Postgres (port 55533) + OpenClaw (port 3579). Config in `toxiproxy.json`. Helper library `scripts/lib/toxiproxy.mjs` provides `addToxic`, `removeToxic`, `resetProxy`, `disableProxy`, `enableProxy`, `waitForToxiproxy`, and preset recipes (`injectLatency`, `injectTimeout`, `injectResetPeer`, `injectBandwidthLimit`). Shared chaos harness in `scripts/lib/chaos-harness.mjs` with `startStack`, `stopStack`, `waitForHealthy`, `insertFakeActiveRun`, `queryRunStatus`, `cleanupTestData`, `dockerCompose`, `dockerComposeBase`. | P4 | Testing infra | **Done** |
| R-14 | **Kill-and-recover integration test** -- `scripts/e2e-chaos-kill-recover.mjs` (`pnpm e2e:chaos:kill-recover`): starts stack, inserts fake active run, kills with SIGKILL, restarts, asserts stale-run sweep detected the orphan (`staleDetectedBy: startup-sweep`), asserts health returns to ok. | P4 | Testing infra | **Done** |
| R-15 | **Postgres restart integration test** -- `scripts/e2e-chaos-postgres-restart.mjs` (`pnpm e2e:chaos:pg-restart`): starts stack, verifies baseline health, restarts postgres container, polls for degraded state, waits for pool reconnection, asserts health and polling recover to ok. | P4 | Testing infra | **Done** |
