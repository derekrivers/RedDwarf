...
- Disabled the OpenClaw gateway-level heartbeat by default. The gateway sends a periodic prompt instructing each agent to read `HEARTBEAT.md` from the workspace root (default 30m interval); RedDwarf does not materialize that file, so VPS logs were spammed with `[tools] read failed: ENOENT: ... /var/lib/reddwarf/workspaces/HEARTBEAT.md` on every wake-up. Added a new `OpenClawHeartbeatConfig` surface in `packages/control-plane/src/openclaw-config.ts` so `generateOpenClawConfig` emits `agents.defaults.heartbeat: { every: "0m" }` by default. Added env knobs `REDDWARF_OPENCLAW_GATEWAY_HEARTBEAT_INTERVAL` (OpenClaw duration syntax; `30m` re-enables) and `REDDWARF_OPENCLAW_GATEWAY_HEARTBEAT_PROMPT` (optional prompt override) wired through `scripts/lib/config.mjs::buildHeartbeatConfigFromEnv` and `scripts/generate-openclaw-config.mjs`. Updated the `infra/docker/openclaw.json` baseline, `.env.example`, and `docs/reference/CONFIG.md`. Two focused regressions added in `packages/control-plane/src/openclaw-config.test.ts`. Verification: `corepack pnpm typecheck`; `corepack pnpm test` (758 passed / 11 skipped); local generator smoke-test confirmed default emits `every: "0m"` and the env override re-enables `every: "30m"`. RedDwarf's phase-level heartbeat (`PHASE_HEARTBEAT_INTERVAL_MS`) is unaffected.
...
- Tightened the bounded-task developer prompt so `spec.md` is the authoritative implementation plan, analyst session history is supplemental when available, and the model is explicitly pushed to start the repo-write path immediately after brief orientation instead of spending its completion budget on silent planning.
- Added regression coverage for mixed-case GitHub repo session keys in developer dispatch and for transcript classification via `sessions.json` fallback when `sessionId` is missing.
...
- Hardened approval resolution for multi-approval tasks. Downstream phases now anchor baseline capability checks to the approved `policy_gate` request instead of whichever approved row happens to sort first, validation separately recognizes approved `architecture_review` overrides when resuming at validation, and approval decision history now records the real request phase instead of hard-coding every approval under `policy_gate`.
- Tightened the ready-dispatch/orphan checks to require an approved `policy_gate` request specifically, so a task with only later override/recovery approvals no longer masquerades as safely dispatchable.
