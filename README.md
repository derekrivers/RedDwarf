...
- developer orchestration, workspace-local validation, and scoped secret leases are available, but code-writing and PR creation remain disabled by default
...
- GitHub and CI integrations are modeled as read-only adapters in v1. Issue intake and status reads are supported; branch, PR, label, workflow, and remote secret mutations stay blocked behind explicit `V1MutationDisabledError` guards, while approved tasks can receive workspace-local scoped secret leases from the secrets adapter.
