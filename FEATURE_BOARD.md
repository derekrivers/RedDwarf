...
### Phase 2 — Approval Rework Flow

| # | Feature | Status | Depends On | Notes |
| - | ------- | ------ | ---------- | ----- |
| 175 | **Approval rework decision for failed phase retry** — Add a third approval decision `"rework"` alongside `approve` and `reject`. When an operator selects rework on a failed-phase escalation (e.g. architecture review failure), the system resets the manifest lifecycle back to the failed phase, attaches the reviewer's feedback and the operator's comments as context for the next agent run, and re-dispatches the phase. The rework decision must: (1) add `"rework"` to `approvalDecisions` in contracts; (2) update `resolveApprovalRequest` to transition the manifest back to `active` at the failed phase instead of `cancelled`; (3) persist the reviewer feedback + operator comment as a `memory_record` with key `rework.feedback:<phase>` so the next development agent sees it in its prompt context; (4) create a new pipeline run for the reworked phase; (5) emit a `REWORK_REQUESTED` run event with the feedback payload; (6) update the dashboard approval UI to show Approve / Rework / Reject buttons (Rework includes a required comment field for operator guidance). The rework flow must respect the existing phase retry budget — if retries are exhausted, rework is not offered. | pending | — | Discovered during E2E project mode testing: architecture reviewer rejected ticket 3 with valid feedback, but the only options were approve (override) or reject (cancel). No way to send the task back for rework with the reviewer's feedback. |

### Phase 3 — Webhook-driven Ticket Advancement

| # | Feature | Status | Depends On | Notes |
| - | ------- | ------ | ---------- | ----- |
| 176 | **Webhook-driven project ticket advancement on PR merge** — Extend the GitHub webhook receiver (added in the webhook intake feature) to handle `pull_request` events with `action: closed` and `merged: true`. When a merged PR is detected, extract the repo and PR number, then call the same `advanceProjectTicket` logic currently triggered by the GitHub Actions workflow (`POST /projects/advance`). This replaces the need for the `reddwarf-advance.yml` workflow file and external `REDDWARF_OPERATOR_API_URL` reachability from GitHub Actions runners. The webhook path already exists at `/webhooks/github` with HMAC verification; this feature adds a second event branch alongside the existing `issues/opened` handler. Must: (1) parse `pull_request` closed+merged events and extract repo + PR number; (2) look up the active project ticket matching the repo and PR; (3) call `advanceProjectTicket` to move the ticket to `merged` and dispatch the next ready ticket; (4) respond 202 Accepted (async, don't block GitHub); (5) log structured events (`WEBHOOK_PR_MERGED`, `WEBHOOK_TICKET_ADVANCED`); (6) gracefully handle PRs that don't match any tracked ticket (200 OK, ignored). Keep the GitHub Actions workflow as a fallback for repos where webhooks aren't configured. | complete | — | Natural extension of the webhook receiver. Would eliminate the GitHub Actions workflow dependency for project ticket advancement, simplifying VPS setup. The `reddwarf-advance.yml` workflow and `REDDWARF_OPERATOR_API_URL` env var can be deprecated once this is stable. |
