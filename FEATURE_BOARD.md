...
| - | ------- | --------- | ------ | ---------- | ---------- | ----- |
| 128 | Add CORS support to the operator API HTTP server | M19 | pending | — | Both | Install `cors` + `@types/cors`; configure with `REDDWARF_DASHBOARD_ORIGIN` env var defaulting to `http://localhost:5173`; add the var to `.env.example` with a comment; apply as the first middleware before all routes. Full spec in `docs/Dashboard.md` §PREREQUISITE — CORS. |
| 129 | Scaffold `packages/dashboard` workspace package with Tabler layout shell and auth | M19 | pending | 128 | Both | Create `packages/dashboard`; register in `pnpm-workspace.yaml` (package name `@reddwarf/dashboard`); React 18 + TypeScript strict + Vite; Vite dev proxy `/api/*` → `http://127.0.0.1:8080`; Tabler admin shell with left sidebar navigation (collapsible), top navbar with app name "RedDwarf Control" + stack health badge + logout, dark/light mode toggle, pending approvals count badge on the Approvals nav item; login screen with single password-type input "Operator Token" that stores to `sessionStorage`; React Router v6 with placeholder routes for all five pages; disabled "Logs" nav item with "Coming soon" Tabler tooltip and commented placeholder route. Full spec in `docs/Dashboard.md` §SCAFFOLD THE PACKAGE and §TABLER LAYOUT SHELL. |
| 130 | Typed API client (`src/api/client.ts`) | M19 | pending | 129 | Both | Typed wrapper around the operator API: `getHealth`, `getPipelineRuns(filters?)`, `getBlockedApprovals`, `getApproval(id)`, `getEvidenceForRun(runId)`, `resolveApproval(id, decision, decisionSummary)`; auto-attach `Authorization: Bearer <token>`; 401 clears token and redirects to login; non-2xx throws typed `ApiError` with status + message; `decidedBy` hardcoded to `"operator"` inside `resolveApproval` — never accepted as a parameter and never exposed in the UI layer. Full spec in `docs/Dashboard.md` §TYPED API CLIENT. |
...
| 132 | Approval detail and resolve page (`/approvals/:id`) — **P1 critical, build before 133–136** | M19 | pending | 131 | Both | On mount fire `getApproval(id)` and `getEvidenceForRun(id)` in parallel; single centred spinner until both resolve; full-page error alert with Retry if either fails. Two-column layout (60/40): left — planning spec in scrollable `<pre>` with syntax-highlighted JSON, task details key/value list, evidence Tabler timeline with expandable raw JSON per event, empty state if no evidence; right sticky — green Approve card (optional note textarea, "Approve Run" button → confirmation modal → `resolveApproval`, success toast + navigate to `/approvals`) and red Reject card (required reason ≥10 chars, submit disabled until valid, same confirmation/toast pattern); both cards show "Processing…" spinner while in-flight; both permanently disabled with a Tabler info alert "This request has already been resolved." if status is not pending (prevents double-submission). Keyboard shortcuts: A → focus approve textarea, R → focus reject textarea, Escape → back to `/approvals`. Full spec in `docs/Dashboard.md` §PRIORITY 1 — Route: /approvals/:id. |
