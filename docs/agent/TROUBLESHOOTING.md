...
## Developer phase appears to "time out" but the real failure was an `elevated`-tool denial (apt-get for a language runtime)

- Symptom: a developer run against a fresh stack (often Ruby/Rails, sometimes Python/Go) appears to hang and is eventually killed. The user-visible failure looks like a timeout; the actual log line is `[tools] exec failed: elevated is not available right now (runtime=direct). Failing gates: enabled (tools.elevated.enabled / agents.list[].tools.elevated.enabled)` with `raw_params` showing a command like `apt-get update && apt-get install -y ruby ruby-dev build-essential libsqlite3-dev pkg-config` and `elevated: true`. Adjacent log noise includes `lane wait exceeded ... waitedMs=~230000 queueAhead=0` and may include `[bundle-mcp] failed to start server "reddwarf" ... Connection closed` plus several `sessions.resolve INVALID_REQUEST No session found` lines.
- Root cause: the OpenClaw container is unprivileged and the runtime denies any tool call with `elevated: true` (and any `apt-get` / `sudo` / `apk` / `yum`). RedDwarf does **not** emit `tools.elevated.*` keys into the generated OpenClaw config (see `packages/control-plane/src/openclaw-config.ts`) — the "fix-it keys" in the error message are OpenClaw's generic suggestion and there is no operator path on the RedDwarf side to flip them. The agent loop silently retries the denied call, the lane sits idle waiting on a tool that will never succeed, and the wall clock eventually kills the run. The MCP "Connection closed" and `sessions.resolve` errors are downstream noise from the stalled session, not the cause.
- Failing approach: enabling `tools.elevated.enabled` somewhere on the gateway, hand-editing OpenClaw config to add the key, or trying alternate package names (`ruby-full` instead of `ruby`). The gate is permanent and unrelated to the package list. Also failing: assuming this is a real timeout and increasing the developer wall-clock budget.
- Working workaround: the toolchain image is mise-based and stack-agnostic by design (see `infra/docker/openclaw/Dockerfile` and `docs/reference/CONFIG.md` `OPENCLAW_IMAGE` notes). Language runtimes must be installed via `mise`, not `apt-get`. Either (a) add a `.tool-versions` file at the repo root pinning the runtime (e.g. `ruby 3.3.0`) and have the developer run `mise install`, or (b) `mise use -g ruby@3.3` for a one-off install. The build chain (`build-essential`, `libssl-dev`, `libpq-dev`, `libsqlite3-dev`, `libxml2-dev`) is already in the image, so native gem extensions build without any system installs. This guidance is now baked into `agents/openclaw/lister/TOOLS.md` (Runtime & Dependency Installation), `agents/openclaw/lister/AGENTS.md` (Known Pitfalls), and `agents/openclaw/holly/AGENTS.md` (Known Pitfalls) so neither agent should plan or execute the apt-get path.
- Verification: rerun the affected ticket. The developer should produce a `.tool-versions` (if missing), then `mise install` should populate `~/.local/share/mise/installs/<lang>/<version>/` and complete in seconds on subsequent tasks (cached via the named volumes in `infra/docker/docker-compose.yml`). The `elevated is not available` line should not reappear in the run log. If a task genuinely needs a new system-level C library that isn't in the image, the correct fix is an operator-side change to `infra/docker/openclaw/Dockerfile` plus `infra/docker/openclaw/build.sh`, not an in-task `apt-get`.
